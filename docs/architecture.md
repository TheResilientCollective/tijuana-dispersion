...
- `StagnationBoxBackend` (working) — calm-night accumulation box (issue #3). Exact box recurrence `C[t] = C[t-1]·e^(−Δt/τ) + C*·(1−e^(−Δt/τ))` with `C* = E·τ/(A·H_mix)` and `H_mix` keyed off Pasquill stability. Receptor-independent v1. The default `gaussian_plume` request now performs per-timestep **regime dispatch**: stagnation hours (the `regime.is_stagnation` classifier) are served by the box, advective hours by the Gaussian plume. `disable_regime_dispatch=True` forces pure Gaussian; `backend="stagnation_box"` forces the box for every hour. With `emission_driver=true` (issue #6) the box's `E_local` becomes a time-varying, **temperature-led** series `E0·Q10^((T−T_ref)/10)` (`TemperatureEmissionParams`); the wind-quadratic volatilization and cosine-diel factors of `emissions.py` are deliberately excluded — paired experiments-repo studies showed them anti-skilled in the trapped calm-night regime while temperature alone clears the constant-box rank-skill ceiling. Box *parameter calibration* (τ, area, and now `Q10`/`T_ref`/`E0`) is an experiments-repo follow-up (no calibration data in this service repo); v1 defaults are uncalibrated.
- `LagrangianPuffBackend` (working, issue #1) — pure-Python (numpy) Gaussian puff model. Each step releases mass-conserving sub-puffs per source; puffs advect by the current wind, σ_y/σ_z grow with travelled path (Briggs), receptor concentration is the summed 3-D puff kernel with ground reflection; out-of-domain puffs are pruned. With σ_x ≡ σ_y the steady-wind sum reduces analytically to the steady-state Gaussian plume (agrees within 10% > 200 m downwind), while cold-start/step changes show the correct delayed-then-rising transient the plume cannot represent. Handles non-stationary releases; in-process. Out of scope (negligible at the 1 h transport scale): deposition, buoyant rise, chemistry.
